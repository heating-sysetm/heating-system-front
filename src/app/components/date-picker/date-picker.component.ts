...
ngOnInit() {
  this.dateForm = new FormGroup({
    start_time: new FormControl(jalaliMoment),
    end_time: new FormControl(jalaliMoment),
  });
}
...
public dateFilter = () => {
  if (!this.dateForm.invalid) {
    this.data_sharing.start_time.next(
      this.format(this.dateForm.value["start_time"], "jYYYY-jMM-jDD")
    );
    this.data_sharing.end_time.next(
      this.format(this.dateForm.value["end_time"], "jYYYY-jMM-jDD")
    );
    console.log(this.data_sharing.start_time.value,this.data_sharing.end_time.value);
    
  } else {
    // this.toastr.error("تاریخ انتخاب شده صحیح نمی باشد", "اخطار!");
  }
};
